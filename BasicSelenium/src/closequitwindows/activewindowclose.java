...
import java.util.*;
public class activewindowclose {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		ArrayList<String> al= new ArrayList(ow); // we are geeting the windows list in order nows
		
		driver.switchTo().window(al.get(1));
		Thread.sleep(3000);
		
		
		driver.switchTo().window(al.get(0)); // moved to parent
		Thread.sleep(3000);
		driver.switchTo().window(al.get(1));// moved to child
		Thread.sleep(3000);
		driver.switchTo().window(al.get(0)); // moved to parent
		Thread.sleep(3000);
		System.out.println(driver.getTitle());
		
		driver.close();  //closes the window where the driver is 
